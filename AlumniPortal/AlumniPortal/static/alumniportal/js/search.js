...
$(document).ready(function () {
    ...
    $("#passout-range-slider").slider({
        min:2012,
        max:2020,
        range: true,
        values: [ 2012, 2020 ],
        slide: function( event, ui ) {
            $('#grad-year-start').html(ui.values[0]);
            $('#grad-year-end').html(ui.values[1]);
        },
        change: filterList
    });
...
function append_filter_html(filter, id) {
    if (document.getElementById(id).innerHTML != "")
        return "&" + filter + "=" + document.getElementById(id).innerHTML;
    return "";
}
...
function filterList() {
    ...
    url += append_filter_html("graduation_year__lte", "grad-year-end");
    url += append_filter_html("graduation_year__gte", "grad-year-start");
